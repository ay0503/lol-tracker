      ...
      <div>
        ...
        <div className="flex gap-1 items-end h-10">
          {Array.from({ length: 15 }).map((_, i) => (
            <div key={i} className="flex-1 h-7 animate-pulse bg-secondary rounded" />
          ))}
        </div>
      </div>
    );
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        >
          <div
            ...
            style={{
              backgroundColor:
                currentStreak.type === "win" ? "var(--color-win)" : "var(--color-loss)",
            }}
          />
          <span
            className="text-sm font-semibold font-[var(--font-mono)]"
            style={{
              color: currentStreak.type === "win" ? "var(--color-win)" : "var(--color-loss)",
            }}
          >
            {currentStreak.count}
        ...
        {sequence.map((result, i) => {
          ...
          return (
            <motion.div
              ...
              initial={{ scaleY: 0 }}
              animate={{ scaleY: 1 }}
              transition={{ type: "spring", damping: 26, stiffness: 260, delay: i * 0.04, duration: 0.3, ease: "easeOut" }}
            ...
            >
              <div
                ...
                style={{
                  backgroundColor: isWin ? "var(--color-win)" : "var(--color-loss)",
            ...
            style={{
              backgroundColor:
                streak.type === "win"
                  ? "rgba(0, 200, 5, 0.08)"
                  : "rgba(255, 82, 82, 0.08)",
              color: streak.type === "win" ? "var(--color-win)" : "var(--color-loss)",
            }}
          >
            {streak.count}
